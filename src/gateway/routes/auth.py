...
async def login(request: LoginRequest, response: Response):
    ...
    try:
        # Llamar a Auth Service
        grpc_response = await auth_client.login(email=request.email, password=request.password)

        # Verificar éxito
        if not grpc_response.success:
            error_msg = (
                grpc_response.error.message if grpc_response.error else "Credenciales inválidas"
            )
